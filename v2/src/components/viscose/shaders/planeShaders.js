...
export const fragmentShader = /* glsl */ `
  ...
  uniform vec4  uHairline;     // card edge: rgb, alpha — the site's --hairline
  ...
  void main() {
    ...
    // The card's hairline, drawn on the field itself rather than into the
    // atlas: a stroke in the texture is square and the corners here are not,
    // so it was being clipped at every corner. One screen pixel deep,
    // measured in the same px as d, so it stays a hairline at any card size
    // and follows the goo where two cards run together.
    float rim = 1.0 - smoothstep(1.0 - aa, 1.0 + aa, -d);
    col = mix(col, uHairline.rgb, rim * uHairline.a);
