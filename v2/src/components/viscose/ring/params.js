...
// The ring borrows the site's own faces and ink instead of bundling its own.
// next/font exposes each family as a CSS variable on <html>; these resolve it
// to a concrete family stack that both DOM styles and canvas ctx.font accept.
...
const family = (name, fallback) => `${cssVar(name, fallback)}, ${fallback}`;
...
export function defaultParams() {
  return {
    ...
    // One plane per project: eleven studies give ~33° slots, close enough
    // to the composition this was designed at that nothing needs re-dealing,
    // and it keeps the list, the cells and the planes one-to-one — which is
    // what lets a click on the column pick its card unambiguously.
    ...
    spinDelay: 0,
    posX: -1.59, // fraction of half the viewport width; lands the front card on centre
    posY: 0,
    endScale: 4.46,
    moveTime: 2.2,
    ...
    text: "Selected work",
    textSize: 41,
    textFont: family("--font-archivo", "\"Helvetica Neue\", Arial, sans-serif"),
    textWeight: 400,
    textTracking: 0, // em
    textColor: cssVar("--ink", "#f2efe5"),
    ...
    nameFont: family("--font-archivo", "\"Helvetica Neue\", Arial, sans-serif"),
    nameWeight: 500,
    idxSize: (16 / 1440) * 100, // a step lighter and smaller than the name
    idxFont: family("--font-jetbrains", "ui-monospace, monospace"),
    idxWeight: 400,
